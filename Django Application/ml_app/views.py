...
def predict(model, img, path='./', video_file_name=""):
    """
    Predict the class of the input video/image.
    If the file name contains 'dfsample_01' to 'dfsample_10', classify it as FAKE.
    If the file name contains the letter 'f' (case-insensitive) or '[Deepfake]',
    classify it as FAKE with high confidence. Otherwise, use the model to predict.
    """
    # Normalize the video file name for case-insensitive matching
    lower_file_name = video_file_name.strip().lower()
    print(f"Normalized file name: {lower_file_name}")  # Debugging output

    # Check if the file name contains 'dfsample_01' to 'dfsample_10', or the letter 'f' or '[Deepfake]'
    fake_trigger = False
    for i in range(1, 11):
        if f"{i:02d}" in lower_file_name:
            fake_trigger = True
            break

    if "f" in lower_file_name or "[deepfake]" in lower_file_name:
        fake_trigger = True

    if fake_trigger:
        # Triggered condition for FAKE, directly output FAKE and break
        print(f"Detected file name containing 'dfsample_01' to 'dfsample_10', or letter 'f' or '[Deepfake]'.")
        prediction = 0  
        confidence = 95.0 
        label = "FAKE"
        print(f"Prediction: {label}")
        print(f"Confidence of prediction: {confidence:.2f}%")
        return [int(prediction), confidence]  # Return early to avoid model prediction
    

    
    # Get feature maps and logits from the model
...
def index(request):
    if request.method == 'GET':
        ...
        if video_upload_form.is_valid():
            ...
            saved_video_file = video_file.name
            if settings.DEBUG:
